...
public class NOpenPartialSynchronizedProduct<T> {
	...
	public NOpenPartialSynchronizedProduct<T> perform() {
		...
		Queue<State> open = new PriorityQueue<>(new WeightBasedStateComparator());

		root = getState(new BitSet(), HashMultiset.create(), HashMultiset.create()).getSecond();
		open.offer(root);
...
//		}
		...
		dfs(root, map.keySet(), HashMultiset.create());
...
//			System.out.println("    Could be a SKIPPED event?");
	...
	private List<LinkedList<Pair<State, Operation>>> dfs1() {
		...
		traverse(differences, new Pair<>(root, null), null, false);
