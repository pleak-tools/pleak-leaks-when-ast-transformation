...
public class PartialSynchronizedProduct<T> {
	...
	static class WeightBasedStateComarator implements Comparator<State> {
	...
	public PartialSynchronizedProduct<T> perform() {
		...
		Queue<State> open = new PriorityQueue<>(new WeightBasedStateComarator());

		root = getState(new BitSet(), HashMultiset.create(), new BitSet()).getSecond();
		open.offer(root);
...
//			System.out.printf("Earliest missmatch: %s, %s\n", earliestMissmatch, earliestMissmatch.nextState);
			if (earliestMissmatch != null) {
				...
				if (labelOperationMap.get(earliestMissmatch.label).size() > 1) {
					...
					PriorityQueue<Operation> leftHidings = new PriorityQueue<>(new Comparator<Operation>() {
						public int compare(Operation o1, Operation o2) {
							return Float.compare(o1.nextState.weight, o2.nextState.weight);
						}
					});
					PriorityQueue<Operation> rightHidings = new PriorityQueue<>(new Comparator<Operation>() {
						public int compare(Operation o1, Operation o2) {
							return Float.compare(o1.nextState.weight, o2.nextState.weight);
						}
					});					
