...
public class NewDiffVerbalizer<T> {
	...
	public NewDiffVerbalizer(PESSemantics<T> pes1, NewUnfoldingPESSemantics<T> pes2, ExpandedPomsetPrefix<T> epp) {
		...
		this.root = new State(new BitSet(), HashMultiset.create(), HashMultiset.create());
	...
	public void verbalize() {
		...
		findCausalityConcurrencyMismatches(root, new HashSet<Pair<State,Operation>>(), new HashSet<>(), new LinkedList<>(), pending);
		if (!pending.isEmpty())
			throw new RuntimeException("Something wrong with some causality/concurrency mismatching events: " + pending);
		findConflictMismatches(root, new HashSet<Pair<State,Operation>>(), new HashSet<Pair<State,Operation>>(), new HashSet<>(), new LinkedList<>());
		markExpandedPrefix(root, HashMultimap.create(), HashMultimap.create(), new HashSet<>(), null);
		findSkipMismatches(root, new HashMap<>(), new HashMap<>(), new HashSet<>());
